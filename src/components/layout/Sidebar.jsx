...
const Sidebar = ({ isCollapsed, toggleSidebar }) => {
    ...
    return (
        <div
            className={`h-screen bg-slate-950 text-white flex flex-col fixed left-0 top-0 border-r border-cyber-border z-50 transition-all duration-300 ${isCollapsed ? 'w-20' : 'w-64'}`}
        >
            {/* Logo Area */}
            <div className={`p-6 flex items-center gap-3 border-b border-cyber-border ${isCollapsed ? 'justify-center' : ''}`}>
                <div className="bg-cyber-primary/20 p-2 rounded-lg shrink-0 border border-cyber-primary/50 shadow-neon">
                    <Droplets size={24} className="text-cyber-primary" />
                </div>
                {!isCollapsed && (
                    <div className="overflow-hidden whitespace-nowrap">
                        <h1 className="text-xl font-bold tracking-tight text-white">Borewell Guard</h1>
                        <p className="text-[10px] uppercase tracking-widest text-cyber-primary font-semibold">Smart Water Monitor</p>
                    </div>
                )}
            </div>
            ...
            <button
                ...
                className="absolute -right-3 top-20 bg-cyber-bg text-cyber-primary p-1 rounded-full shadow-neon-border border border-cyber-primary/30 hover:bg-cyber-primary/10 transition-colors z-50 flex items-center justify-center w-6 h-6"
            >
                {isCollapsed ? (
